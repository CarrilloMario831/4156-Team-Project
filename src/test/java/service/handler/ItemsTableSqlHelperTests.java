...
public class ItemsTableSqlHelperTests {
  ...
  @InjectMocks
  private ItemsTableSqlHelper itemsTableSqlHelper;
  ...
  public void testInsertItem() {
    ...
    assertThrows(RuntimeException.class, () -> itemsTableSqlHelper.insertItem(testItem),
  ...
  public void testGetAllItems() {
    ...
    assertThrows(RuntimeException.class, () -> itemsTableSqlHelper.getAllItems(),
  ...
  public void testGetItem() {
    ...
    items.add(testItem);
    when(jdbcTemplate.query(anyString(), any(RowMapper.class))).thenReturn(items);
    assertEquals(items, itemsTableSqlHelper.getItem(itemId),
            "Should return the list of items " +
            "with the given ID.");

    // Test item not found
    when(jdbcTemplate.query(anyString(), any(RowMapper.class))).thenReturn(new ArrayList<>());
    assertTrue(itemsTableSqlHelper.getItem(itemId).isEmpty(),
            "Should return an empty list when item is not found.");

    // Test exception thrown
    when(jdbcTemplate.query(anyString(), any(RowMapper.class))).thenThrow(RuntimeException.class);
    assertThrows(RuntimeException.class, () -> itemsTableSqlHelper.getItem(itemId),
            "Should propagate exceptions.");
  }
  ...
  public void testUpdateItemLocation() {
    ...
    assertTrue(itemsTableSqlHelper.updateItemLocation(itemId, newLocation),
    ...
    assertFalse(itemsTableSqlHelper.updateItemLocation(itemId, newLocation),
            "Update should return false when no rows are affected.");

    // Test exception thrown
    when(jdbcTemplate.update(anyString(), eq(newLocation), eq(itemId)))
            .thenThrow(RuntimeException.class);
    assertThrows(RuntimeException.class, () -> itemsTableSqlHelper
            .updateItemLocation(itemId, newLocation), "Should propagate exceptions.");
  }
  ...
  public void testUpdateItemPrice() {
    ...
    assertTrue(itemsTableSqlHelper.updateItemPrice(itemId, newPrice),
            "Update should return true when one row is affected.");

    // Test unsuccessful update
    when(jdbcTemplate.update(anyString(), eq(newPrice), eq(itemId))).thenReturn(0);
    assertFalse(itemsTableSqlHelper.updateItemPrice(itemId, newPrice),
            "Update should return false when no rows are affected.");
  ...
  public void testUpdateItemQuantity() {
    ...
    assertTrue(itemsTableSqlHelper.updateItemQuantity(itemId, newQuantity),
            "Update should return true when one row is affected.");

    // Test unsuccessful update
    when(jdbcTemplate.update(anyString(), eq(newQuantity), eq(itemId))).thenReturn(0);
    assertFalse(itemsTableSqlHelper.updateItemQuantity(itemId, newQuantity),
            "Update should return false when no rows are affected.");
  ...
  public void testDeleteItem() {
    ...
    assertTrue(itemsTableSqlHelper.deleteItem(itemId),
    ...
    assertFalse(itemsTableSqlHelper.deleteItem(itemId),
    ...
    assertThrows(RuntimeException.class, () -> itemsTableSqlHelper.deleteItem(itemId),
            "Should propagate exceptions.");
  }
  ...
  public void testUpdateItemName() {
    ...
    assertTrue(itemsTableSqlHelper.updateItemName(itemId, newItemName),
            "Update should return true when one row is affected.");

    // Test unsuccessful update
    when(jdbcTemplate.update(anyString(), eq(newItemName), eq(itemId))).thenReturn(0);
    assertFalse(itemsTableSqlHelper.updateItemName(itemId, newItemName),
            "Update should return false when no rows are affected.");
